...
device_t::device_t()
  : command_handlers(command_t::MAX_COMMANDS), // MAX_COMMANDS 값은 256
...
{ // cmd.cmd 메서드는 tohost >> 48 값을 반환
  command_handlers[cmd.cmd()](cmd); // command_handlers에 cmd 인자를 전달하여 실행
}
...
{
  size_t what = cmd.payload() % command_t::MAX_COMMANDS; // cmd.payload 메서드는 tohost << 16 >> 16 값을 반환
