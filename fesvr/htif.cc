...
htif_t::htif_t(const std::vector<std::string>& args) : htif_t()
{
  ...
  for (unsigned int i = 0; i < args.size(); i++) {
    argv[i+1] = (char *) args[i].c_str(); // args의 요소들을 한 칸씩 뒤로 이동
  }
  //Set line size as 16 by default.
  line_size = 16;
  parse_arguments(argc, argv); // targs에 ["./pk/pk", "/bench/test1"] 포함
  register_devices();
}
...
{
  std::string path;
  if (access(payload.c_str(), F_OK) == 0) // "./pk/pk" 파일의 존재 여부 확인
    path = payload;
...
{
  std::map<std::string, uint64_t> symbols = load_payload(targs[0], &entry); // targs[0]은 "./pk/pk"
  // symbols에 symbol 이름을 키로 해당 symbol 주소 저장. entry에 entry point(프로그램 시작 주소) 저장
  ...
  for (auto payload : payloads) // payloads는 빈 벡터
  {
    reg_t dummy_entry;
   ...
   for (auto i : symbols)
   { // .second는 std::map<std::string, uint64_t>에서 두번째 요소
      auto it = addr2symbol.find(i.second);
      if (it == addr2symbol.end()) // addr2symbol에 존재하지 않는 경우
...
int htif_t::run() // simulator 실행
{
  start();

  auto enq_func = [](std::queue<reg_t>* q, uint64_t x) { q->push(x); }; // lamda 함수
  std::queue<reg_t> fromhost_queue;
  std::function<void(reg_t)> fromhost_callback =
    std::bind(enq_func, &fromhost_queue, std::placeholders::_1); // enq_func 함수의 첫 인자에 fromhost_queue를 bind

  if (tohost_addr == 0) { // tohost_addr = symbols["tohost"]
    while (true)
      idle();
  }
  ...
  {
    if (auto tohost = from_target(mem.read_uint64(tohost_addr))) { // tohost_addr에서 byte 데이터 읽어오기
      mem.write_uint64(tohost_addr, target_endian<uint64_t>::zero); // tohost_addr에 0으로 채우기
      command_t cmd(mem, tohost, fromhost_callback); // command_t 객체 생성
...
void htif_t::parse_arguments(int argc, char ** argv)
{
  ...
  while (1) {
    ...
    if (c == -1) break; // argv = ["htif", "./pk/pk", "/bench/test1"]이고, 이들은 long_options에 정의된 옵션이 아니므로
 retry:
    switch (c) {
      case 'h': usage(argv[0]);
