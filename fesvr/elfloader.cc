...
{
  int fd = open(fn, O_RDONLY); // fn은 "./pk/pk"
  struct stat s;
  assert(fd != -1);
  if (fstat(fd, &s) < 0) // fd와 관련된 파일 정보를 s에 저장
    abort();
  size_t size = s.st_size;

  char *buf = (char *)mmap(NULL, size, PROT_READ, MAP_PRIVATE, fd, 0); // 파일을 메모리에 mapping
...
// endr_t: ELF Header type, phdr_t: Program Header type, shdr_t: Section Header type, sym_t: Symbol Table Entry type
#define LOAD_ELF(ehdr_t, phdr_t, shdr_t, sym_t, bswap)                         \
  ...
  } while (0) // entry에 entry point(프로그램 시작 주소) 저장
  // symbols에 symbol 이름을 키로 해당 symbol 주소 저장. 예) symbols["main"] = 0x400000
  if (IS_ELFLE(*eh64)) {
