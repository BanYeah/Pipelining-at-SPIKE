...
{
  func = f; // sim_thread_main 메서드
  arg = a;
  creator = current(); // 현재 실행 context 가져오기

#ifdef USE_UCONTEXT
  getcontext(context.get()); // 현재 실행 context를 context에 저장
  context->uc_link = creator->context.get(); // 복구용 context 저장
  context->uc_stack.ss_size = 64*1024; // context가 사용하는 stack
  context->uc_stack.ss_sp = new void*[context->uc_stack.ss_size/sizeof(void*)]; // stack pointer
#ifndef GLIBC_64BIT_PTR_BUG
...
void context_t::switch_to()
{
  ...
  context_t* prev = cur;
  cur = this;
  if (swapcontext(prev->context.get(), context.get()) != 0) // 현재 실행 context를 저장하고 새로운 context로 전환
    abort();
#else
  cur->flag = 0;
  this->flag = 1;
