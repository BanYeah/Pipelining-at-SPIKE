...
public:
  ...
  int64_t i_imm() { return int64_t(b) >> 20; } // I-type imm
  int64_t shamt() { return x(20, 6); } // shamt
  int64_t s_imm() { return x(7, 5) + (xs(25, 7) << 5); } // S-type imm
  int64_t sb_imm() { return (x(8, 4) << 1) + (x(25,6) << 5) + (x(7,1) << 11) + (imm_sign() << 12); } // B-type imm
  int64_t u_imm() { return int64_t(b) >> 12 << 12; } // U-type imm
  int64_t uj_imm() { return (x(21, 10) << 1) + (x(20, 1) << 11) + (x(12, 8) << 12) + (imm_sign() << 20); } // J-type imm
  uint64_t rd() { return x(7, 5); } // rd
  uint64_t rs1() { return x(15, 5); } // rs1
  uint64_t rs2() { return x(20, 5); } // rs2
