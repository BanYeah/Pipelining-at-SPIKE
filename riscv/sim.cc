...
sim_t::sim_t(const char *isa, const char *priv, const char *varch,
    ...
    : htif_t(args),          // htif_args로 htif_t 객체 생성
      mems(mems),
...
{
  ((sim_t*)arg)->main(); // arg 인자로 main 메서드 호출
}

void sim_t::main()
{
  if (!debug && log)
    set_procs_debug(true); // 각 프로세스의 debug 값을 true로 변경

  while (!done()) // 프로그램 종료 여부 반환(sim_t::run() 끝에 stop 메서드 호출 시 stopped가 true가 되며, done 메서드가 stopped를 반환)
  {
...
int sim_t::run()
{
  host = context_t::current(); // 현재 실행 context 가져오기
...
void sim_t::idle()
{
  target.switch_to(); // target context로 전환
}
