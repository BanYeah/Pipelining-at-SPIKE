...
{
  const disasm_insn_t* disasm_insn = lookup(insn); // 명령어 집합에 insn이 있는지 확인
  return disasm_insn ? disasm_insn->to_string(insn) : "unknown"; // 예) "mv a5, a0"
}
...
const disasm_insn_t* disassembler_t::probe_once(insn_t insn, size_t idx) const
{
  for (size_t j = 0; j < chain[idx].size(); j++)
    if(*chain[idx][j] == insn) // chain[idx]는 명령어 집합
      return chain[idx][j];
...
{
  if (auto p = probe_once(insn, hash(insn.bits(), MASK1))) // MASK1 = 0x7f = 0b0111111: opcode 추출
    return p;
...
void NOINLINE disassembler_t::add_insn(disasm_insn_t* insn) // 명령어 집합(chain) 생성
{
  size_t idx =
