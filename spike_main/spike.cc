...
static void help(int exit_code = 1)
{
  ...
  fprintf(stderr, "usage: spike [host options] <target program> [target options]\n"); // ./build/spike -l ./pk/pk /bench/test1
  fprintf(stderr, "Host Options:\n"); // -l is [host options], ./pk/pk is <target program>, /bench/test1 is [target options]
...
int main(int argc, char** argv)
{
  ...
  parser.option('l', 0, 0, [&](const char* s){log = true;}); // -l 옵션 확인
#ifdef HAVE_BOOST_ASIO
  ...
  auto argv1 = parser.parse(argv); // [host options] 존재 여부를 확인 및 action 실행 후, <target program> 위치에서 반환
  std::vector<std::string> htif_args(argv1, (const char*const*)argv + argc); // <target program>과 [target options]를 htif_arg 벡터에 각각 저장
  if (mems.empty())
    mems = make_mems("2048");
